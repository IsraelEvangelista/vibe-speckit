---
name: byterover
description: "Manages project knowledge using ByteRover context tree. Provides operations: query (retrieve) and curate (store). Use when you need pattern discovery, context lookup, or to persist decisions/bugfixes."
---
...
ByteRover is operated through its CLI (REPL + commands). Authentication and project initialization happen in the terminal, not via an in-app "API connection".
...
Notes:
- Docs: the CLI expects an interactive local environment (TTY + browser auth + OS credential store). Running on headless servers can hang during authentication.
- Linux: ensure `libsecret` is installed so the CLI can store credentials.
...
## Standalone Commands (Automation-Friendly)

Some commands also work outside the REPL:

```bash
brv status
brv query "..."
brv curate "..." -f path/to/file
```

Use `--headless` when running without a TTY. For `brv init --headless`, you must pass `--team` and `--space`.

## Security

- Never curate secrets (`.env`, `.mcp.local.json`, private keys, API keys).
- Prefer authenticating from a trusted terminal (not inside embedded web terminals) when pasting keys/codes.

## Prerequisites

Run `brv status` first. If you see `Not logged in` or `Project Status: Not initialized`, complete the Official Setup above.
